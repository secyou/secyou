...
import com.busicomjp.sapp.repository.b.B5Repository;

@Service
public class B5Service extends B4_5_6AbstractService {
	private final String CONST_REPORT_NAME = "買掛金集計表";
	...
	public List<TorihikisakiSummaryData> selectAccountsSummary(String companyCode, String startYearMonth,
		...
		// 前期繰越
		List<Map<String, Object>> carryForwardMoneyList = b5Repo.selectCarryForwardMoney(selectCondition);
		Map<String, Long> carryForwardMoneyMap = _getCarryForwardMoneyOfSuppliers(carryForwardMoneyList);

		// 最終取引日
		List<Map<String, Object>> lastTradingDateList = b5Repo.selectLastTradingDate(selectCondition);
		Map<String, String> lastTradingDateMap = _getLastTradingDateOfSuppliers(lastTradingDateList);

		// 月ごとの集計
		List<AccountsSummaryData> searchResultList = b5Repo.selectAccountsPayableSummary(selectCondition);
