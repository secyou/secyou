...
import com.busicomjp.sapp.repository.b.B4Repository;

@Service
public class B4Service extends B4_5_6AbstractService {
	private final String CONST_REPORT_NAME = "売掛金集計表";
	private final String CONST_ACCOUNT_TYPE_NYUUKIN = "入金";
	@Autowired
	B4Repository b4Repo;
	...
	public List<TorihikisakiSummaryData> selectAccountsSummary(String companyCode, String startYearMonth,
		...
		// 前期繰越
		List<Map<String, Object>> carryForwardMoneyList = b4Repo.selectCarryForwardMoney(selectCondition);
		Map<String, Long> carryForwardMoneyMap = _getCarryForwardMoneyOfSuppliers(carryForwardMoneyList);
		
		// 最終取引日
		List<Map<String, Object>> lastTradingDateList = b4Repo.selectLastTradingDate(selectCondition);
		Map<String, String> lastTradingDateMap = _getLastTradingDateOfSuppliers(lastTradingDateList);

		// 月ごとの集計
		List<AccountsSummaryData> searchResultList = b4Repo.selectAccountsReceivableSummary(selectCondition);
